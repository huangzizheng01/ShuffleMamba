...
def get_args_parser():
    ...
    parser.add_argument('--ssr', type=float, default=0.1, metavar='PCT',
                        help='stochastic shuffle rate (default: 0.1)')
...
def main(args):
    ...
    # log about
    run_name = str(args.output_dir).split("/")[-1]
    ...
    if args.finetune:
        ...
        for k in ['head.weight', 'head.bias', 'cls_token', 'pos_embed_cls',
                  'neck.weight', 'norm_neck.weight']:
    ...
    if not args.unscale_lr:
        linear_scaled_lr = args.lr * eff_batch_size / 512.0
    ...
    if not args.amp:
        ...
        if args.amp == "bfloat16":
            amp_autocast = torch.cuda.amp.autocast(dtype=torch.bfloat16)
        else:
