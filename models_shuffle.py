...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        self.pretrain_resolution = self.patch_embed.grid_size
        ...
        # self.ssr = [0.] + [shuffle_rate] * (depth-1)  # stochastic depth decay rule
...
@register_model
def mambar_middle_patch16_224(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=576, depth=32, rms_norm=False, residual_in_fp32=True,
        fused_add_norm=False, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
...
if __name__ == '__main__':
    ...
    model = mambar_large_patch16_224_40().cuda()
