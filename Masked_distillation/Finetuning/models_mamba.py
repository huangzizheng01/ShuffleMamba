...
class Block(nn.Module):
    ...
    def forward(
        ...
        hidden_states = hidden_states + self.drop_path(self.mixer(self.norm1(hidden_states), inference_params=inference_params))
...
def create_block(
    ...
    mixer_cls = partial(Mamba, expand=1, layer_idx=layer_idx, bimamba_type=bimamba_type, if_devide_out=if_devide_out, init_layer_scale=init_layer_scale, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
class ARM(nn.Module):
    def __init__(self, 
        ...
        print(self.pos_embed.shape[1], 'ft model pos num')
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        for i, layer in enumerate(self.layers):
            ...
            # )
            hidden_states = shuffle_forward(hidden_states, layer, inference_params=inference_params, 
                                prob = self.ssr[i], training = self.training)
            # print("-------------------------")
        hidden_states = self.norm_f(hidden_states)

        if self.global_pool:
            # print('global_pooling!!!')
            return hidden_states[:, 1:, :].mean(1)
        else:
            # print('cls_pooling')
...
def arm_base_pz16(pretrained=False, **kwargs):
    model = ARM(
        patch_size=16, embed_dim=768, depth=12, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, 
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=False, **kwargs)
...
def arm_large_pz16(pretrained=False, **kwargs):
    model = ARM(
        patch_size=16, embed_dim=1024, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False,
...
def arm_huge_pz16(pretrained=False, **kwargs):
    model = ARM(
        patch_size=16, embed_dim=1536, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, 
        final_pool_type='mean', if_abs_pos_embed=True, if_rope=False, if_rope_residual=False, 
        if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
    model.default_cfg = _cfg()
    if pretrained:
